...
function taskToForm(task) {
  ...
  return {
    ...
    program: task.program ?? "GIA",
    isPriority: Boolean(task.is_priority),
    remarks: meta.cleanRemarks,
  };
}

const EditUserTaskModal = ({ isOpen, onClose, onSuccess, task, currentUserId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await updateTask(task.id, {
      ...
      isPriority,
      remarks,
      existingRemarks: task.existing_remarks ?? task.remarks,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="user-edit-task-modal-title"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl border border-slate-200">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2
              id="user-edit-task-modal-title"
              className="text-xl font-semibold text-slate-900"
            >
              Edit task
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Update task details. Use Request status in the table to change
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label
                htmlFor="user-edit-modal-task-date"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Date
              </label>
              <input
                id="user-edit-modal-task-date"
                type="date"
                required
                value={form.taskDate}
            ...
            <div>
              <label
                htmlFor="user-edit-modal-task-deadline"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Deadline{" "}
              ...
              <input
                id="user-edit-modal-task-deadline"
                type="date"
              ...
              <label
                htmlFor="user-edit-modal-task-deadline-time"
              ...
              <input
                id="user-edit-modal-task-deadline-time"
                type="time"
                step={1800}
            ...
            <div>
              <label
                htmlFor="user-edit-modal-task-program"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Program
              </label>
              <select
                id="user-edit-modal-task-program"
                required
                value={form.program}
            ...
            <div className="sm:col-span-2">
              <label
                htmlFor="user-edit-modal-task-agenda"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Agenda
              </label>
              <input
                id="user-edit-modal-task-agenda"
                type="text"
                required
                value={form.agenda}
            ...
            <div className="sm:col-span-2">
              <label
                htmlFor="user-edit-modal-task-activities"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Activities{" "}
                <span className="font-normal text-slate-400">(optional)</span>
            ...
            <div>
              <label
                htmlFor="user-edit-modal-task-responsible"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Persons responsible
              </label>
              <div
                id="user-edit-modal-task-responsible"
            ...
            <div className="sm:col-span-2">
              <label
                htmlFor="user-edit-modal-task-remarks"
                className="mb-1.5 block text-sm font-medium text-slate-700"
              >
                Remarks
              </label>
              <textarea
                id="user-edit-modal-task-remarks"
                rows={2}
                value={form.remarks}
                onChange={setField("remarks")}
                placeholder="What you plan to do or progress notes for admin"
                className={inputClass}
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button
              type="submit"
              disabled={
                submitting || loadingAssignees || assigneeOptions.length === 0
              }
